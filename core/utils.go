...
var (
	...
	// burst
	initBurst = 1024
	limiter   = rate.NewLimiter(rate.Limit(initLimit), initBurst)
)

// IO read count
var ReadCount int64 = 0

// UpdateIoLimiterLimit update limit
func UpdateIoLimiterLimit(newLimit int) {
	if newLimit > 0 {
		limiter.SetLimit(rate.Limit(newLimit))
	}
}

// UpdateIoLimiterBurst update burst
func UpdateIoLimiterBurst(newBurst int) {
	if newBurst > 0 {
